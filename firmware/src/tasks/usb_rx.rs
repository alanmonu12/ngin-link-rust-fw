...
use crate::app_context::{CAN_CMD_CHANNEL, CanDriverCmd, USB_RX_READY};

#[embassy_executor::task]
pub async fn usb_rx_task(mut ep_out: bsp_f446::usb::BspUsbEndpointOut) {
    ep_out.wait_enabled().await;
    USB_RX_READY.signal(());
    trace!("USB RX: Endpoint OUT habilitado");
    ...
    loop {
        match ep_out.read(&mut buf).await {
            Ok(n) if n >= core::mem::size_of::<GsTxMsg>() => {
                ...
                trace!("USB RX: TX request id=0x{:03X}, ext={}, dlc={}, echo_id={}", 
                    tx_msg.id(), tx_msg.is_extended(), tx_msg.dlc(), tx_msg.echo_id);
                let tx_req = crate::app_context::parse_tx_msg(&tx_msg);
